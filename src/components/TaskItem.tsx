...
import { useConfettiStars } from "./ConfettiStars";
...
const TaskItem: React.FC<TaskItemProps> = (props) => {
  ...
  const { triggerConfetti } = useConfettiStars();
  ...
  const handleToggle = (
    ...
    try {
      // Only animate when marking task as complete
      if (!task.completed) {
        // Get task position for confetti placement
  ...
  return (
    <div
      className={`p-3 bg-white dark:bg-gray-700 rounded-lg shadow-sm border hover:shadow-md transition-all duration-2000 ${
        isSelected
